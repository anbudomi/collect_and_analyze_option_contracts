...
class DataAnalyzer:
    def __init__(self, prepared_db_path, indices):
        ...
        self.bool = 0  # Legt fest, welche Analyse ausgeführt wird
    ...
    def run_analysis_and_plotting(self):
        ...
        # Je nach Einstellung können verschiedene Analysen ausgeführt werden
        if self.bool == 1:
    ...
    def abs_rel_deviation_in_2018_and_2019(self):
        ...
        for ticker in self.indices:
            table_name = f"prepared_{ticker.lower()}_data"
            query = f"SELECT date, relative_error, absolute_error FROM {table_name}"
            df = query_db(self.prepared_db_path, query)
            ...
            print(f"\nMonatliche durchschnittliche absolute Abweichung ({ticker}):")
            print(df_monthly[['absolute_error']])
            print("\n" + "-" * 50 + "\n")

        # Liniendiagramme erstellen
        fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(12, 8), sharex=True)

        # Plot für relative Abweichung
        for ticker, df_monthly in data_dict.items():
            ...
            axes[0].plot(df_monthly.index, df_monthly['relative_error'], label=label, color=color,
                         marker='o', linestyle='-', linewidth=0.5, alpha=0.5)
            # Punkte mit voller Deckkraft
            axes[0].plot(df_monthly.index, df_monthly['relative_error'], color=color,
                         marker='o', linestyle='None', alpha=1)
        axes[0].set_title("Monatliche durchschnittliche relative Abweichung (2018-2019)")
        axes[0].set_ylabel("Relative Abweichung (%)")
        leg0 = axes[0].legend()
        axes[0].grid(True, linestyle='--', alpha=0.7)

        # Plot für absolute Abweichung
        for ticker, df_monthly in data_dict.items():
            if ticker.upper() == "NDX":
                label = "Nasdaq 100"
                color = "navy"
                ...
                label = ticker
                color = None
            axes[1].plot(df_monthly.index, df_monthly['absolute_error'], label=label, color=color,
                         marker='o', linestyle='-', linewidth=0.5, alpha=0.5)
            axes[1].plot(df_monthly.index, df_monthly['absolute_error'], color=color,
                         marker='o', linestyle='None', alpha=1)
        axes[1].set_title("Monatliche durchschnittliche absolute Abweichung (2018-2019)")
        ...
        # Speichere den Plot im gewünschten Verzeichnis
        save_path = os.path.join(self.plots_dir, "Relative und absolute Abwiechung in 2018_2019.png")
        plt.savefig(save_path)
        print(f"Plot gespeichert unter: {save_path}")
